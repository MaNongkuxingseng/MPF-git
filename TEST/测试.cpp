...
void Append(ListNode** pHead,int Value)
{
...
void RemoveNode(ListNode** pHead,int Value)
{
	...
	else
	{
		...
		{
			while(pNode->pNext->m_Value != Value && pNode->pNext != NULL)
			{
				pNode = pNode->pNext;
			}
			if (pNode->pNext->m_Value == Value)
			{
				delete pNode;
				pNode = pNode->pNext->pNext;
			}
			else
				cout<<"can't find "<<Value<<"in the link."<<endl;
		}
	}
}
